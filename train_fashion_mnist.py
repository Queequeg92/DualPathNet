...
from fashion_mnist import MNIST
...
parser = argparse.ArgumentParser(description='PyTorch Fashion-Mnist Classification Training')
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        MNIST('MNIST_data', dataset='train', download=False, force_download=False,
    ...
    test_loader = torch.utils.data.DataLoader(
        MNIST('MNIST_data', dataset='test', download=False,
              transform=transforms.Compose([transforms.RandomCrop(28, padding=4),
                                            transforms.RandomHorizontalFlip(),
                                            transforms.ToTensor(),
                                            transforms.Normalize((0.1307,), (0.3081,))])),
...
def adjust_learning_rate(optimizer, epoch):
    if args.lr_schedule == 0:
        ...
        lr = args.lr * ((0.1 ** int(epoch >= 10)) * (0.1 ** int(epoch >= 20)))
    else:
